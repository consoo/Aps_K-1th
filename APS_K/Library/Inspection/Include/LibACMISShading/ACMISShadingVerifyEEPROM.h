...
enum ECOLORSHADING_MAP { ECOLORSHADING_CENTER = 1, ECOLORSHADING_CORNER = -1 };

class ACMISSHADING_API CACMISShadingVerifyEEPROM
{
public:
	CACMISShadingVerifyEEPROM(void);
	~CACMISShadingVerifyEEPROM(void);
	...
	ERetMsg SymmetricEx(const unsigned short* pEepromBuf, const char* pMap, double dSpecShadingMin, double dSpecCornerDeltaMin, double *dShadingMin, double *dShadingDeltaMin);
	ERetMsg CurvatureEx(const unsigned short* pEepromBuf, const char* pMap, int nSpecCurvatureDepth, int nSpecCurvatureCnt, int* CurvatureDepth, int* CurvatureCnt);
};
