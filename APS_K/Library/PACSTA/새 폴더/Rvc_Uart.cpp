...
{
	int portNum = sysData.iCommPort[COMM_RVC];
	CString portStr;
	portStr.Format("\\\\.\\COM%d", 7);// portNum);
	theApp.MainDlg->putListLog(portStr);
	int ret = CSerialComm::GetInstance().Open(portNum, 416000, 8, ONESTOPBIT, EVENPARITY);
...
int CRvc_Uart::DisableSharpness(IAutomotiveCamera* pCamera)
{
	...
	unsigned char writeDataAES[3] = { 0x02, 0x00, 0x60 };
